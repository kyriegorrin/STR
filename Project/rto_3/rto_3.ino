...
unsigned char exec_array [150][9];
...
void printaContexts(){
    ...
    for(int i = 0; i < TASKS; ++i){
      for(int j = 0; j < num_contexts; ++j){
        ...
        Serial.print(": ");
        Serial.print(*((char *)&exec_array[j]));
        Serial.print(" ");
      }
    Serial.println("");
  }
...
void taskTracer(void * pvParameters) {
  ...
  for (;;) {
    ...
    //Guardant temps actual per a un futur
    //AQUESTA LINIA FA PETAR ELS PRINTS PER SERIAL WTF
    //exec_array[num_contexts]timing = current_time;

    //Pointer for access to context array
    //char *context_pointer = (char *) &exec_array[num_contexts];
    
		for (i=0; i<TASKS; i++) {  
      //Aquesta primera linia peta l'execucio per alguna rao
      exec_array[num_contexts][i] = (unsigned char)(status_array[i].uxCurrentPriority);
